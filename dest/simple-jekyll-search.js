...
(function () {
  ...
  function findMatchesInObject(obj, crit, strategy, opt) {
    ...
    // modify search critera to not include tags or categories
    tags.forEach(function (item, index) { //remove tags from search critera
      crit = crit.split(item).join('');
    });
    categories.forEach(function (item, index) { //remove tags from search critera
      crit = crit.split(item).join('');
    });
    ...
    for (var key in obj) {
      if (tags.length && key === "tags") {
        if (!isExcluded(obj[key], opt.exclude) && !strategy.matches(obj[key], tags)) { //if no tags match then end search by return
          return
        }
      } else if (categories.length && key === "category") {
        if (categories && !isExcluded(obj[key], opt.exclude) && !strategy.matches(obj[key], categories)) { //if no categories match then end search by return
          return
        }
      } else if (contents) {

        if (!isExcluded(obj[key], opt.exclude) && strategy.matches(obj[key], crit)) { //if find match in some content
          match = true;
        }
      }
    }
